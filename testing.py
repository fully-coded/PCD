...
def select_image():
    ...
    if len(path) > 0:
        ...
        ret, thresh_image = cv2.threshold(
            imgfiltered, 100, 255, cv2.THRESH_BINARY_INV)
        ...
        circles = cv2.HoughCircles(
            morpho, cv2.HOUGH_GRADIENT, 1, 20, param1=50, param2=30, minRadius=350, maxRadius=500)
        ...
        for cnt in contours0:
            if cv2.contourArea(cnt) < pupil_area:
                ...
                label4 = Label(
                    root, text="You have %.2f percent cataract" % (cataract_percentage))
        ...
        img_morpho_copy = img_morpho_copy.resize(
